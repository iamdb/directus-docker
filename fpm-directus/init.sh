...
if [ -f /var/www/html/api/config.php ] && [ -n "${DIRECTUS_ENV}" ] && [ ${DIRECTUS_ENV} != "development" ]; then
  ...
  su-exec www-data sed -i "s|'MEMCACHED_ENV_NAMESPACE', 'staging'|'MEMCACHED_ENV_NAMESPACE', '${DIRECTUS_ENV}'|g" /var/www/html/api/config.php
fi
...
if [ -f /var/www/html/api/configuration.php ] && [ -n "${S3_KEY}" ] && [ -n "${S3_SECRET}" ] && [ -n "${S3_REGION}" ] && [ -n "${S3_BUCKET}" ]; then
    ...
    su-exec www-data sed -i "s|'root_url' => '/storage/uploads'|'root_url' => 'https://s3.amazonaws.com/${S3_BUCKET}${S3_ROOT}'|g" /var/www/html/api/configuration.php
    su-exec www-data sed -i "s|'root_thumb_url' => '/storage/uploads/thumbs'|'root_thumb_url' => 'https://s3.amazonaws.com/${S3_BUCKET}${S3_ROOT}/thumbs'|g" /var/www/html/api/configuration.php
